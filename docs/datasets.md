...
- **fires**: Distribution of wildfire polygons. The layer must have the following two attributes: (1) "YEAR" - an integer  which is the ignition year of the fire e.g., 1995 and (2) "CAUSE" - cause of ignition of the fire which can either be 'Human', 'Natural' or 'Unknown'.  
The application provides fires as follow by default: NRCAN Natural Burned Area Composite (https://cwfis.cfs.nrcan.gc.ca/downloads/nbac/) for the yeras of 1972-2025 and NRCAN National Fire Database Perimeters (https://cwfis.cfs.nrcan.gc.ca/downloads/nfdb/fire_poly/current_version/) prior to 1972.
...
- **intact_fl_2025**: Distribution of intact forest landscapes in the year 2025 (Potapov et al. 2017). Source: https://intactforests.org
- footprint_500m: Combination of linear disturbances and areal disturbances provided by the application buffered with 500m.
- undisturbed_areas_500m: Areas within the study area that are not intersected by the footprint_500m layer.
...
- caribou_herds: Caribou herds annual distribution (YG),
